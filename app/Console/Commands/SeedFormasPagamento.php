...
class SeedFormasPagamento extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'tenant:seed-formas-pagamento {--tenant= : ID específico do tenant} {--all : Executar em todos os tenants}';
     ...
     * @var string
     */
    protected $description = 'Executar seeder de formas de pagamento em tenants';
    ...
    {
        $this->info('🌱 Iniciando seed de formas de pagamento...');
        ...
        $this->info('✅ Seed de formas de pagamento concluído!');
        return 0;
    }
    ...
    private function seedTenant(Tenant $tenant): void
    {
        $tenant->run(function () use ($tenant) {
            $this->info("  🌱 Executando seeder de formas de pagamento para: {$tenant->name}");

            try {
                // Executar apenas o seeder de formas de pagamento
                Artisan::call('db:seed', [
                    '--class' => 'FormasPagamentoSeeder',
                    '--force' => true,
                    '--quiet' => true
                ]);
