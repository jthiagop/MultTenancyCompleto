    <!--end::Card head-->
    <form id="kt_modal_new_target_form" class="form" action="{{ route('caixa.store') }}" method="POST"
        ...
        <div class="card">
            ...
            <div class="card-header card-header-stretch">
                ...
                <div class="card-title d-flex align-items-center">
                    <h3 class="fw-bold m-0 text-gray-800"> Informações do lançamento</h3>
                </div>
                <!--end::Title-->
                <!--begin::Toolbar-->
                        ...
                        <div class="card-body pt-0">
                            ...
                            <div class="row g-9 mb-8 fv-row">
                                ...
                                <div class="col-md-4 fv-row">
                                    ...
                                    <!--end::Label-->
                                    <input type="text" value="{{ old('descricao') }}"
                                        class="form-control form-control-solid" placeholder="Descrição do lançamento"
                                        name="descricao" />
                                </div>
                                <!--begin::Col-->
                                <div class="col-md-3 fv-row">
                            ...
                            <div class="row g-9 mb-8 fv-row">
                                ...
                                <div class="col-md-5 fv-row">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <select name="lancamento_padrao_id" id="lancamento_padrao_caixa"
                                            ...
                                            <option value=""></option>
                                            <!-- As opções de <option> podem ser montadas dinamicamente via JavaScript -->
                                            <!-- mas também podem ser geradas em Blade se você preferir. -->
                                            @foreach ($lps as $lp)
                                                <option value="{{ $lp->id }}"
                                ...
                                <div class="col-md-4 fv-row">
                                    ...
                                    <div class="input-group">
                                        <select name="cost_center_id" id="banco_id"
                                    ...
                                    <select id="bancoSelect" name="entidade_banco_id" aria-label="Select a Banco"
                                        ...
                                        @foreach ($entidadesBanco as $entidade)
                                            <option value="{{ $entidade->id }}"
                                                ...
                                                {{ $entidade->nome }} ({{ ucfirst($entidade->agencia) }})
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('entidade_banco_id')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                            </div>
                                    ...
                                    <div class="tab-content" id="myTabContent">
                                        <div class="tab-pane fade show active" id="kt_tab_pane_1" role="tabpanel">
                                            <textarea class="form-control" name="historico_complementar" id="complemento" cmaxlength="250" rows="3"
                                                name="target_details" placeholder="Mais detalhes sobre lançamento"></textarea>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // 2. Mostra/oculta o campo "Banco de Depósito"
            const toggleBancoDepositoField = (description) => {
                if (description === 'Deposito Bancário') {
                    $('#banco-deposito').fadeIn();
                } else {
                    $('#banco-deposito').fadeOut();
                }
            };

            // 3. Recria as opções do select "Lançamento Padrão" dependendo do tipo (entrada ou saída)
            const reloadLancamentoPadraoOptions = (tipo) => {
                ...
                // Percorre lpsData e adiciona só as que batem com "tipo"
                lpsData.forEach(function(lp) {
                    if (lp.type === tipo) {
                        const option = document.createElement('option');
                        option.value = lp.id;
                        option.text = lp.description;
                        option.setAttribute('data-description', lp.description);
                        lancamentoPadraoSelect.appendChild(option);
                    }
                });
            };

            // 4. Evento "change" no tipo (Entrada/Saída)
            const tipoSelect = document.getElementById('tipo_select_banco');
            tipoSelect.addEventListener('change', function() {
                const selectedTipo = tipoSelect.value;

                // Recarrega as opções de Lançamento Padrão
                reloadLancamentoPadraoOptions(selectedTipo);

                // Reinicializa o Select2
                $('#lancamento_padrao_caixa').select2('destroy');
                initializeSelect2();

                // Esconde o campo banco-deposito (até selecionar novamente)
                $('#banco-deposito').hide();
            });

            // 5. Evento "change" no Lançamento Padrão
            $('#lancamento_padrao_caixa').on('change', function() {
                const selectedOption = $(this).find('option:selected');
                const selectedText = selectedOption.text().trim();

                toggleBancoDepositoField(selectedText);
            });

            // --------------------------------------
            // A. Lógica para reexibir valores ao dar "old()" (erro de validação)
            // --------------------------------------

            // (A.1) Se existir old('tipo'), forçamos a recarga inicial das opções
            //       e selecionamos a <option> antiga de lancamento_padrao_id
            let oldTipo = '{{ old('tipo') }}'; // Vem do Blade
            if (oldTipo === 'entrada' || oldTipo === 'saida') {
                // Recarrega opções de Lançamento Padrão para esse tipo
                reloadLancamentoPadraoOptions(oldTipo);
            }

            // Inicializa ou reinicializa o select2
            initializeSelect2();

            // (A.2) Caso exista um old('lancamento_padrao_id'), o Blade já definirá <option selected>.
            //       Agora, verificamos se o texto selecionado é “Deposito Bancário” para exibir o campo.
            let selectedOption = $('#lancamento_padrao_caixa').find('option:selected');
            let selectedTextOnLoad = selectedOption.text().trim();

            toggleBancoDepositoField(selectedTextOnLoad);
        });
    </script>
