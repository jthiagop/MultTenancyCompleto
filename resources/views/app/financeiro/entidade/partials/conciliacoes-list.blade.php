{{-- 
    Partial: Lista de Conciliações
    Reutilizável para as 3 tabs (Todos, Recebimentos, Pagamentos)
    Recebe: $conciliacoesPendentes, $entidade, $centrosAtivos, $lps, $formasPagamento
--}}
...
@else
                                ...
                                <div class="d-flex flex-stack pb-3">
                                    ...
                                    <div class="d-flex">
                                        <div class="text-end pb-3">
                                            @if ($transacaoSugerida && $transacaoSugerida->tipo == 'entrada')
                                                <span class="fw-bold fs-5" style="color: green;">R$ {{ number_format($transacaoSugerida->valor / 100, 2, ',', '.') }}</span>
                                            @elseif($transacaoSugerida && $transacaoSugerida->tipo == 'saida')
                                                <span class="fw-bold fs-5 text-danger">R$ {{ number_format($transacaoSugerida->valor / 100, 2, ',', '.') }}</span>
                                            @else
                                ...
                                <form class="edit-suggestion-form row" data-conciliacao-id="{{ $conciliacao->id }}" method="POST"
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="text" class="form-control" name="valor"
                                            value="{{ old('valor', $transacaoSugerida ? number_format($transacaoSugerida->valor / 100, 2, ',', '.') : number_format($conciliacao->amount, 2, ',', '.')) }}">
                                    </div>
    ...
    <!-- Botões de paginação -->
    {{ $conciliacoesPendentes->appends(['tab' => request('tab', 'all')])->links('pagination::bootstrap-5') }}
@endif
