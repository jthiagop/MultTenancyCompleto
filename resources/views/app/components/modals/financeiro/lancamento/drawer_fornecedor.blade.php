...
<div class="rounded-4 " id="kt_drawer_fornecedor" data-kt-drawer="true" data-kt-drawer-activate="true"
    ...
    <div class="card shadow-sm w-100">
        ...
        <div class="card-body drawer-body py-10 px-lg-17">
            ...
            <form id="kt_drawer_fornecedor_form">
                ...
                <div class="row">
                    ...
                    <!--end::Col-->
                    <!--begin::Natureza (Fornecedor / Cliente)-->
                    <div class="col-12">
                        <label class="fs-6 fw-semibold mb-2">Natureza</label>
                        <div class="d-flex gap-5">
                            <div class="form-check">
                                <input class="form-check-input natureza-check" type="checkbox" value="fornecedor" id="check_fornecedor" />
                                <label class="form-check-label" for="check_fornecedor">
                                    Fornecedor
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input natureza-check" type="checkbox" value="cliente" id="check_cliente" />
                                <label class="form-check-label" for="check_cliente">
                                    Cliente
                                </label>
                            </div>
                        </div>
                        <div class="fv-plugins-message-container invalid-feedback d-none" id="natureza_error">
        ...
        <div class="card-footer">
            ...
            <button type="submit" class="btn btn-sm btn-primary" id="kt_drawer_fornecedor_submit"
                form="kt_drawer_fornecedor_form">
...
@push('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // ========== LÓGICA DOS CHECKBOXES NATUREZA ==========
            const checkFornecedor = document.getElementById('check_fornecedor');
            const checkCliente = document.getElementById('check_cliente');
            const naturezaHidden = document.getElementById('parceiro_natureza_hidden');
            const naturezaError = document.getElementById('natureza_error');

            function atualizarNatureza() {
                const isFornecedor = checkFornecedor && checkFornecedor.checked;
                const isCliente = checkCliente && checkCliente.checked;

                if (isFornecedor && isCliente) {
                    naturezaHidden.value = 'ambos';
                } else if (isFornecedor) {
                    naturezaHidden.value = 'fornecedor';
                } else if (isCliente) {
                    naturezaHidden.value = 'cliente';
                } else {
                    naturezaHidden.value = '';
                }

                // Limpa erro ao selecionar
                if (naturezaError && (isFornecedor || isCliente)) {
                    naturezaError.classList.remove('d-block');
                    naturezaError.classList.add('d-none');
                    if (checkFornecedor) checkFornecedor.classList.remove('is-invalid');
                    if (checkCliente) checkCliente.classList.remove('is-invalid');
                }
            }

            if (checkFornecedor) checkFornecedor.addEventListener('change', atualizarNatureza);
            if (checkCliente) checkCliente.addEventListener('change', atualizarNatureza);

            // Define estado inicial dos checkboxes baseado no título do drawer
            function aplicarNaturezaPadrao() {
                const drawerTitle = document.getElementById('fornecedor_drawer_title');
                const titulo = drawerTitle ? drawerTitle.textContent.trim().toLowerCase() : '';

                if (titulo.includes('fornecedor')) {
                    if (checkFornecedor) checkFornecedor.checked = true;
                    if (checkCliente) checkCliente.checked = false;
                } else if (titulo.includes('cliente')) {
                    if (checkFornecedor) checkFornecedor.checked = false;
                    if (checkCliente) checkCliente.checked = true;
                }
                atualizarNatureza();
            }

            // Aplica ao carregar e observa mudanças no título
            aplicarNaturezaPadrao();
            const tituloDrawer = document.getElementById('fornecedor_drawer_title');
            if (tituloDrawer) {
                new MutationObserver(function() {
                    setTimeout(aplicarNaturezaPadrao, 200);
                }).observe(tituloDrawer, { childList: true, characterData: true, subtree: true });
            }
            // ========== FIM LÓGICA NATUREZA ==========
                ...
                checkDocumentoTimeout = setTimeout(function() {
                    fetch('{{ route('parceiros.check-documento') }}', {
                        ...
                        .then(data => {
                            ...
                            const existingFeedback = inputElement.parentElement.querySelector(
                                '.invalid-feedback');
                            ...
                            if (data.exists) {
                                ...
                                feedback.innerHTML =
                                    `<i class="bi bi-exclamation-triangle me-1"></i>${data.message} <strong>(${data.parceiro?.nome || 'Parceiro'})</strong>`;
            ...
            if (formFornecedor) {
                formFornecedor.addEventListener('submit', function(e) {
                    if (cnpjDuplicado || cpfDuplicado) {
                        ...
                        e.stopPropagation();
                        toastr.error(
                            'Não é possível salvar: CPF ou CNPJ já cadastrado para outro parceiro.');
                        return false;
                    }
                });
            }
            ...
            if (btnConsultar) {
                btnConsultar.addEventListener('click', function() {
                    ...
                    btnConsultar.innerHTML =
                        '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Consultando...';
                    ...
                    fetch('{{ route('company.consultar-cnpj') }}', {
                        ...
                        .then(data => {
                            ...
                            if (document.getElementById('nome_completo')) {
                                ...
                                document.querySelector('input[name="razao_social"]').value = data
                                    .razao_social || '';
                            }
                            ...
                            if (document.getElementById('fornecedor_nome')) {
                                if (!document.getElementById('fornecedor_nome').value) {
                                    document.getElementById('fornecedor_nome').value = data
                                        .nome_fantasia || data.razao_social || '';
                                }
                            }
                            ...
                            if (data.telefone) {
                                if (document.getElementById('fornecedor_telefone')) {
                                    document.getElementById('fornecedor_telefone').value = data
                                        .telefone;
                                }
                            }
                            ...
                            if (document.querySelector('input[name="address1"]')) {
                                document.querySelector('input[name="address1"]').value = data
                                    .logradouro || '';
                            } else if (document.getElementById('logradouro')) {
                                document.getElementById('logradouro').value = data.logradouro || '';
                            }

                            if (document.getElementById('numero')) document.getElementById('numero')
                                .value = data.numero || '';
                            if (document.getElementById('bairro')) document.getElementById('bairro')
                                .value = data.bairro || '';
                            ...
                            if (data.uf) {
                                ...
                                if (targetSelect) {
                                    if (typeof $ !== 'undefined' && $(targetSelect).hasClass(
                                            'select2-hidden-accessible')) {
                            ...
                            fetch(`https://viacep.com.br/ws/${cep}/json/`)
                                ...
                                .then(data => {
                                    if (!("erro" in data)) {
                                        ...
                                        if (document.querySelector('input[name="city"]')) {
                                            document.querySelector('input[name="city"]').value = data
                                                .localidade;
                                        }
                                        ...
                                        const selectCountry = document.querySelector(
                                            'select[name="country"]');
                                        if (selectCountry) {
                                            if (typeof $ !== 'undefined' && $(selectCountry).hasClass(
                                                    'select2-hidden-accessible')) {
            ...
            if (form) {
                form.addEventListener('submit', function(e) {
                    ...
                    // Validação natureza: ao menos um checkbox marcado
                    const checkForn = document.getElementById('check_fornecedor');
                    const checkCli = document.getElementById('check_cliente');
                    const natErr = document.getElementById('natureza_error');
                    if (checkForn && checkCli && !checkForn.checked && !checkCli.checked) {
                        if (natErr) {
                            natErr.classList.remove('d-none');
                            natErr.classList.add('d-block');
                        }
                        if (checkForn) checkForn.classList.add('is-invalid');
                        if (checkCli) checkCli.classList.add('is-invalid');
                        return;
                    }
                            ...
                            if (result.success) {
                                ...
                                if (novoId && novoNome && typeof $ !== 'undefined') {
                                    ...
                                    if ($target.length) {
                                        ...
                                        console.log('[DrawerFornecedor] Select atualizado:',
                                            targetSelectId, 'com:', novoNome, '(ID:', novoId, ')');
                                    } else {
                                        console.warn('[DrawerFornecedor] Select alvo não encontrado:',
                                            targetSelectId);
                                        ...
                                        if (targetSelectId === '#cliente_id') {
                                            ...
                                            if ($fallback.length) {
                                                ...
                                                const optFallback = new Option(novoNome, novoId, true,
                                                    true);
                                                $fallback.append(optFallback).trigger('change');
                                                console.log(
                                                    '[DrawerFornecedor] Fallback para #fornecedor_id com:',
                                                    novoNome);
                                            }
                                        }
                                    }
                                    ...
                                    console.warn(
                                        '[DrawerFornecedor] Dados incompletos para atualizar select:', {
                                            novoId,
                                            novoNome
                                        });
                                }
                                ...
                                $('#cnpj_container .invalid-feedback, #cpf_container .invalid-feedback')
                                    .remove();
