<div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
    <!--begin:Form-->
    <form id="kt_modal_foro_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" method="POST"
        action="{{ route('patrimonio.store') }}">
        @csrf

        {{-- Meta tag CSRF já presente --}}
        <meta name="csrf-token" content="{{ csrf_token() }}">

        <input type="hidden" id="method_field" name="_method" value="POST">
        <input type="hidden" id="address_id" name="id">

        {{-- Adicione um campo hidden para a ação de salvamento (usado pelo JS) --}}
        ...
        <div class="row g-9 mb-8">
            ...
            <div class="col-md-8 fv-row">
                ...
                <input type="hidden" id="numForo" name="numForo">
                <input type="hidden" id="numIbge" name="numIbge">
                {{-- Seu script JS para atualizar os campos ocultos --}}
                <script>
                    // É melhor mover este script para o seu arquivo JS principal
                    // ou colocá-lo dentro do KTUtil.onDOMContentLoaded
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        if (patrimonioSelect) {
                            patrimonioSelect.addEventListener('change', function() {
                                ...
                                const numForo = selectedOption.getAttribute('data-num-foro');
                                const numIbge = selectedOption.getAttribute('data-ibge'); // Corrigido para data-ibge
                                document.getElementById('numForo').value = numForo || ''; // Define como vazio se nulo
                                document.getElementById('numIbge').value = numIbge || ''; // Define como vazio se nulo
                            });
                        }
                    });
                </script>
        ...
        <div class="text-center">
            <button type="reset" id="kt_modal_new_foro_cancel" class="btn btn-light me-3">Sair</button>

            <!-- Botão principal (ENVIO NORMAL) -->
            <div class="btn-group dropup">
                <!-- Botão principal: Envia normal e recarrega a página -->
                ...
                <!-- Botão de dropup -->
                <button type="button"
                    class="btn btn-primary dropdown-toggle dropdown-toggle-split border-start border-white border-opacity-25"
                    {{-- Classes adicionadas aqui --}} data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <span class="visually-hidden">Toggle Dropdown</span> {{-- Usar visually-hidden em vez de sr-only no BS5 --}}
                </button>

                <!-- Opções do dropup -->
                <div class="dropdown-menu">
                    <!-- Salvar e Clonar -->
                    <a class="dropdown-item" href="#" id="kt_modal_new_foro_clone">Salvar e Clonar</a>

                    <!-- Salvar e em Branco -->
                    <a class="dropdown-item" href="#" id="kt_modal_new_foro_novo">Salvar e Limpar</a>
                </div>
            </div>
        </div>
