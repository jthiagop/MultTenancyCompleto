...
interface DataGridColumnHeaderProps<TData, TValue> extends HTMLAttributes<HTMLDivElement> {
  ...
  title?: string;
  /** Ícone à esquerda do título. Omitir usa o ícone padrão do tipo de coluna (`column.id`); `null` oculta. */
  icon?: ReactNode | null;
  pinnable?: boolean;
  filter?: ReactNode;
  visibility?: boolean;
...
function DataGridColumnHeader<TData, TValue>({
  ...
  const ColumnGlyph = getDataGridColumnVisibilityIcon(column.id);
  const leadingIcon: ReactNode =
    icon === undefined ? (
      <ColumnGlyph aria-hidden className="size-3.5 shrink-0 opacity-60" />
    ) : (
      icon
    );
            ...
            {props.tableLayout?.columnsPinnable && column.getCanPin() && (
              <>
                <DropdownMenuItem onClick={() => column.pin(column.getIsPinned() === 'left' ? false : 'left')}>
                  ...
                  <span className="grow">Coluna à Esquerda</span>
                ...
                <DropdownMenuItem onClick={() => column.pin(column.getIsPinned() === 'right' ? false : 'right')}>
                  ...
                  <span className="grow">Coluna à Direita</span>
                ...
                <DropdownMenuPortal>
                  <DropdownMenuSubContent>
                    {table
                      ...
                      .map((col) => {
                        const Icon = getDataGridColumnVisibilityIcon(col.id);
                        const label = getDataGridColumnVisibilityLabel(col.id, col.columnDef.meta?.headerTitle);
                        return (
                          <DropdownMenuCheckboxItem
                            key={col.id}
                            className="gap-2"
