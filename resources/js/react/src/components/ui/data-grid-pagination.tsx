...
function DataGridPagination(props: DataGridPaginationProps) {
  ...
  const btnPageClasses  = 'size-7 min-w-7 p-0 text-xs tabular-nums';
  const btnArrowClasses = 'size-7 p-0 rtl:transform rtl:rotate-180';
    ...
    for (let i = currentGroupStart; i < currentGroupEnd; i++) {
      ...
      buttons.push(
        <Button
          ...
          onClick={() => {
            if (!active) table.setPageIndex(i);
          }}
        >
          {i + 1}
        </Button>,
    ...
    if (currentGroupStart > 0) {
      return (
        <Button
          size="sm"
          variant="ghost"
          className={cn(btnPageClasses, 'text-muted-foreground')}
    ...
    >
      <div className="flex flex-wrap items-center space-x-2.5 pb-2.5 sm:pb-0 order-2 sm:order-1">
        {isLoading ? (
          ...
          <>
            <div className="text-sm text-muted-foreground">Registros por página</div>
            <Select
              value={`${pageSize}`}
