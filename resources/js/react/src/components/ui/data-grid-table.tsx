...
function getPinningStyles<TData>(column: Column<TData>, isAutoWidth = false): CSSProperties {
  ...
  return {
    ...
    // No modo auto a coluna cresce com o conteúdo; o size vira apenas um mínimo
    ...(isAutoWidth ? { minWidth: size } : { width: size }),
    zIndex: isPinned ? 1 : 0,
  };
}
  ...
  return (
    <th
      ...
      style={{
        ...
        ...(props.tableLayout?.columnsPinnable && column.getCanPin() && getPinningStyles(column, props.tableLayout?.width === 'auto')),
  ...
  return (
    <tr
      ...
      onClick={(e) => {
        ...
        if ((e.target as HTMLElement).closest('[data-row-click-ignore="true"]')) return;
...
function DataGridTableRowSelect<TData>({ row, size }: { row: Row<TData>; size?: 'sm' | 'md' | 'lg' }) {
  return (
    <div data-row-click-ignore="true" className="inline-flex items-center" onClick={(e) => e.stopPropagation()}>
      <div
