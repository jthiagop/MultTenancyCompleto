...
const ACCOUNT_BADGE_CLASS: Record<string, string> = {
...
function entidadeBadges(
  tipo: string,
  accountType?: string | null,
  agenciaConta?: string,
): Array<{ label: string; variant?: 'secondary' | 'outline'; className?: string }> {
  const typeKey = tipo === 'caixa' ? 'caixa' : (accountType ?? '');
  const colorClass = ACCOUNT_BADGE_CLASS[typeKey] ?? 'bg-muted text-muted-foreground';
  const typeLabel = tipo === 'caixa' ? 'Caixa' : (ACCOUNT_TYPE_LABELS[accountType ?? ''] ?? 'Banco');

  const result: Array<{ label: string; variant?: 'secondary' | 'outline'; className?: string }> = [
    { label: typeLabel, className: colorClass },
  ];
  if (agenciaConta) {
