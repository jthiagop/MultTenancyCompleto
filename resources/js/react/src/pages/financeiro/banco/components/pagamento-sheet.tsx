...
import { Loader2, X, Pencil, History } from 'lucide-react';
          ...
          <ScrollArea className="h-full">
            <div className="p-5 space-y-5">
              {loading ? (
                ...
                <>
                  {/* ── Informações do lançamento (readonly) ──────────────── */}
                    ...
                    <CardContent className="p-4">
                      <div className="grid grid-cols-3 gap-x-6 gap-y-3 text-xs">
                        <div>
                          ...
                          <span className="font-medium">{transacao.parceiro_nome || '—'}</span>
                        </div>
                        <div>
                          <span className="text-muted-foreground block">Cód. referência</span>
                          <span className="font-medium">{transacao.numero_documento || '—'}</span>
                        </div>
                        <div>
                          <span className="text-muted-foreground block">Data de competência</span>
                          <span className="font-medium">{fmtDate(transacao.data_competencia)}</span>
                        </div>
                        <div>
                          <span className="text-muted-foreground block">Categoria</span>
                          <span className="font-medium flex items-center gap-1">
                            {transacao.categoria_nome || '—'}
                            {transacao.categoria_nome && <Pencil className="size-3 text-muted-foreground" />}
                          </span>
                        </div>
                        <div>
                          <span className="text-muted-foreground block">Centro de custo</span>
                          <span className="font-medium flex items-center gap-1">
                            {transacao.centro_custo_nome || '—'}
                            {transacao.centro_custo_nome && <Pencil className="size-3 text-muted-foreground" />}
                          </span>
                        </div>
                        <div>
                          <span className="text-muted-foreground block">Recorrente</span>
                          <span className="font-medium">{transacao.recorrencia_id ? 'Sim' : 'Não'}</span>
                        </div>
                      </div>

                      <div className="border-t border-border mt-3 pt-3 grid grid-cols-4 gap-x-6 text-xs">
                        <div>
                          <span className="text-muted-foreground block">Vencimento</span>
                          <span className="font-medium">{fmtDate(transacao.data_vencimento)}</span>
                        </div>
                        <div>
                          <span className="text-muted-foreground block">Parcela</span>
                          <span className="font-medium">{transacao.parcelamento || 'À vista'}</span>
                        </div>
                        <div>
                          <span className="text-muted-foreground block">Descrição</span>
                          <span className="font-medium flex items-center gap-1">
                            {transacao.descricao || '—'}
                            {transacao.descricao && <Pencil className="size-3 text-muted-foreground" />}
                          </span>
                        </div>
                        <div className="text-right">
                          <span className="text-muted-foreground block">Valor total</span>
                          <span className="text-lg font-bold">R$ {fmtBrl(transacao.valor)}</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
