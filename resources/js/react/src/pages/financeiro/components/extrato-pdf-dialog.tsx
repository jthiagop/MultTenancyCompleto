...
interface ExtratoPdfDialogProps {
  open: boolean;
...
/** Converte 'YYYY-MM-DD' → 'DD/MM/YYYY' esperado pelos jobs existentes */
function isoToBr(iso: string): string {
  const [y, m, d] = iso.split('-');
  return `${d}/${m}/${y}`;
}

export function ExtratoPdfDialog({ open, onOpenChange }: ExtratoPdfDialogProps) {
  ...
  // Carrega contas ao abrir ou ao trocar tipo
  useEffect(() => {
    if (!open) return;
  ...
  async function handleSubmit() {
    ...
    setLoading(true);
    try {
      const res = await fetch('/relatorios/extrato/pdf-async', {
        method: 'POST',
        headers: {
      ...
      if (!res.ok || !json.success) {
        notify.error('Erro', json.message ?? 'Não foi possível gerar o extrato.');
        return;
      }
      onOpenChange(false);
      notify.pdfLoading('Extrato em processamento', 'O PDF está sendo gerado e chegará nas suas notificações em instantes.');
    } catch {
      notify.error('Erro', 'Não foi possível gerar o extrato.');
    } finally {
      setLoading(false);
    }
  }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-prose !top-[8%] !translate-y-0 min-h-[320px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Selecione a conta e o período para gerar o extrato financeiro.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogBody className="flex flex-col gap-5 py-2">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Select
              ...
              disabled={loadingContas || entidadesFiltradas.length === 0}
            >
              <SelectTrigger>
                <SelectValue
                  placeholder={
