...
import { Bell, FileDown, Loader2 } from 'lucide-react';
...
interface OFXExportDialogProps {
  open: boolean;
...
/** Converte 'YYYY-MM-DD' → 'DD/MM/YYYY' esperado pelo OfxExportService */
function isoToBr(iso: string): string {
  const [y, m, d] = iso.split('-');
  return `${d}/${m}/${y}`;
}

export function OFXExportDialog({ open, onOpenChange }: OFXExportDialogProps) {
  ...
  async function handleSubmit() {
    ...
    setLoading(true);
    try {
      const res = await fetch('/relatorios/ofx/exportar-async', {
        method: 'POST',
        headers: {
      ...
      if (!res.ok || !json.success) {
        notify.error('Erro', json.message ?? 'Não foi possível gerar o OFX.');
        return;
      }
      onOpenChange(false);
      notify.success('OFX em processamento', 'O arquivo OFX está sendo gerado e chegará nas suas notificações em instantes.');
    } catch {
      notify.error('Erro', 'Não foi possível gerar o OFX.');
    } finally {
      setLoading(false);
    }
  }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-prose !top-[8%] !translate-y-0 min-h-[320px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Selecione a conta e o período para gerar o arquivo OFX (Open Financial Exchange).
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogBody className="flex flex-col gap-5 py-2">
          ...
          <div className="flex gap-3 rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950/40">
            ...
            <span className="text-xs text-amber-600/80 dark:text-amber-400/80">
              O arquivo OFX será gerado em segundo plano e chegará nas suas notificações quando estiver pronto.
            </span>
          </div>
        </DialogBody>
