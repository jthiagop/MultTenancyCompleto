  ...
  const columns = useMemo<ColumnDef<ITransacao>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="font-semibold tabular-nums">{fmtCurrency(row.original.valor)}</span>
        ),
        enableSorting: true,
        size: 130,
      },
      {
        ...
        header: ({ column }) => <DataGridColumnHeader title="A Receber (R$)" column={column} />,
        cell: ({ row }) => {
          const v = row.original.valor_restante;
          return (
            <span className={`font-semibold tabular-nums ${v <= 0 ? 'text-success' : ''}`}>
              {fmtCurrency(v)}
            </span>
          );
        },
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const { situacao_label, situacao_color } = row.original;
          const variant = SITUACAO_VARIANT[row.original.situacao] ?? situacao_color ?? 'secondary';
          return (
      ...
      <Card>
        <SummaryStatsBar
          ...
          stats={[
            ...
            { statKey: 'recebidos',  label: 'Recebidos (R$)',        value: statsContas?.recebidos  ?? 0, colorClass: 'text-emerald-500', accentColor: '#10b981' },
