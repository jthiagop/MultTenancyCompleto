...
function pathMatchesQuery(pathWithQuery: string, pathname: string, search: string): boolean {
  ...
  const pathOk =
    pathname === pathPart ||
    (pathPart.length > 1 && pathPart !== '/' && pathname.startsWith(pathPart));
  if (!pathOk) {
    return false;
  }
...
export const useMenu = (pathname: string, search: string = ''): UseMenuReturn => {
  const isActive = (path: string | undefined): boolean => {
    if (!path) {
      return false;
    }
    if (path.includes('?')) {
      return pathMatchesQuery(path, pathname, search);
    }
    if (path === '/') {
