...
>(
  (
    ...
    const effectiveState = state === "collapsed" && isHovered ? "expanded" : state
    ...
    return (
      ...
      >
        {/* This is what handles the sidebar gap on desktop - always uses real state, not effectiveState */}
        <div
          className={cn(
        ...
        <div
          className={cn(
            ...
            // Higher z-index when hovered to overlay content
            isHovered && state === "collapsed" ? "z-1050" : "z-1050",
            className
          )}
          {...props}
        >
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        "h-6 w-6 rounded-md border bg-background shadow-sm hover:bg-accent", // Small square button look
        className
      )}
      onClick={(event) => {
        onClick?.(event)
      ...
      {...props}
    >
      <ArrowLeftToLine className={cn("h-4 w-4 transition-transform duration-200", state === "collapsed" ? "rotate-180" : "")} />
...
// icon size group label altura icon 
