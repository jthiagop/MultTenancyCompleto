...
const initDropzone = () => {
    ...
    // Certifique-se de que o contêiner do Dropzone existe na página
    if (!dropzone) return;

    // Obtém o token CSRF do Laravel
    const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');

    // Define o template para pré-visualização de arquivos
    let previewNode = dropzone.querySelector(".dropzone-item");
    previewNode.id = "";
    let previewTemplate = previewNode.parentNode.innerHTML;
    previewNode.parentNode.removeChild(previewNode);

    // Inicializa o Dropzone com configurações personalizadas
    const myDropzone = new Dropzone(id, {
        url: `/anexos/${caixaId}/update`, // Corrige a URL para a rota correta
        method: 'POST',
        headers: {
    ...
    // Evento para enviar todos os arquivos na fila
    dropzone.querySelector(".dropzone-upload").addEventListener('click', function () {
        if (myDropzone.getQueuedFiles().length > 0) {
            // Processa a fila de arquivos
            myDropzone.processQueue();
        } else {
            // Mostra uma mensagem se nenhum arquivo estiver na fila
            Swal.fire({
    ...
    // Evento antes de enviar os arquivos
    myDropzone.on("sending", function (file, xhr, formData) {
        formData.append('_method', 'PUT');
    });

    // Evento quando todos os arquivos são enviados com sucesso
    myDropzone.on("queuecomplete", function () {
        Swal.fire({
            ...
            location.reload(); // Recarrega a página após o upload
        });
    });

    // Evento para remover todos os arquivos
    ...
    // Evento quando um arquivo é removido manualmente
...
// Chama a função para inicializar o Dropzone
document.addEventListener('DOMContentLoaded', function () {
    initDropzone();
});


// Inicializa o Dropzone quando o DOM estiver completamente carregado
document.addEventListener('DOMContentLoaded', function () {
    initDropzone();
});
