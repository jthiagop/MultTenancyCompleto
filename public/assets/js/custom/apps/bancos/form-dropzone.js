...
var KTAppEcommerceSaveProduct = function () {
    ...
    const initDropzone = () => {
        ...
        var myDropzone = new Dropzone("#kt_ecommerce_add_product_media", {
            ...
            maxFilesize: 10, // MB
            addRemoveLinks: true,
            acceptedFiles: "image/*,application/pdf,.doc,.docx,.txt", // Aceita imagens, PDFs, Word e arquivos de texto
            init: function () {
                // Configuração para enviar os arquivos ao formulário
                this.on("success", function (file, response) {
                    console.log("Arquivo enviado com sucesso:", file, response);
                });

                this.on("error", function (file, errorMessage) {
                    console.log("Erro no envio do arquivo:", errorMessage);
                });
            },
                    // Função de aceitação personalizada para excluir arquivos executáveis
        accept: function(file, done) {
            const executableTypes = ['.exe', '.bat', '.sh', '.cmd']; // Tipos de arquivos executáveis
            const fileExtension = file.name.split('.').pop().toLowerCase(); // Obter a extensão do arquivo

            if (executableTypes.includes('.' + fileExtension)) {
                done("Arquivos executáveis não são permitidos.");
            } else {
                done();
            }
        }
