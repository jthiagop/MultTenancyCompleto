...
var initDatatable = function () {
    ...
    tableRows.forEach(row => {
        ...
        const realDate = moment(dateRow[0].innerHTML, "MMM DD, YYYY").format(); // select date from 1st column in table
        dateRow[0].setAttribute('data-order', realDate);
    });

    // Init datatable --- more info on datatables: https://datatables.net/manual/
    datatable = $(table).DataTable({
        "info": false,      // Oculta informações como "Mostrando X a Y de Z entradas"
    ...
    return {
        init: function () {
            table = document.querySelector('#DM_Entidade_Financeira_table');
